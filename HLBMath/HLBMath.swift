...
@main
struct HLBMath: ParsableCommand {
...
extension HLBMath {
    static func format(_ result: Int, usingHex: Bool) -> String {
        usingHex ? String(result, radix: 16) : String(result)
    }
...
extension HLBMath.Statistics {
    ...
    struct Average: ParsableCommand {
        ...
        @Option(help: "The kind of average to provide(mean, median, mode).")
        var kind: Kind = .mean
    ...
    // 分位数(分位点), 这个子命令的功能好像没写完.
    // https://github.com/apple/swift-argument-parser/blob/main/Examples/math/main.swift
